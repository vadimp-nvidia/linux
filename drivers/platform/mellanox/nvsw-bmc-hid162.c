...
struct nvsw_bmc_device {
	struct nvsw_core core;
	struct i2c_mux_regmap_platform_data mux_pdata[NVSW_MUX_MAX];
};
...
static int nvsw_bmc_hid189_chan1[] = {
	0x01, 0x02, 0x0a, 0x07, 0x09, 0x03, 0x04, 0x08, 0x0c, 0x0d, 0x15,
};
...
static int nvsw_bmc_hid162_mux_topology_init(struct nvsw_core *nvsw_core)
{
	...
	for (i = 0; i < nvsw_core->mux_num; i++) {
		nvsw_core->mux_pdata[i].regmap = nvsw_core->regmap;
		nvsw_core->mux[i] = platform_device_register_resndata(nvsw_core->dev,
								      "i2c-mux-regmap", i, NULL, 0,
								      &nvsw_core->mux_pdata[i],
								      sizeof(nvsw_core->mux_pdata[i]));
...
fail_platform_mux_register:
	while (--i >= 0) {
		if (!IS_ERR_OR_NULL(nvsw_core->mux[i])) {
			platform_device_unregister(nvsw_core->mux[i]);
			nvsw_core->mux[i] = NULL;
		}
	}
	return err;
}
...
static int nvsw_bmc_hid162_apply_hw_defaults(struct regmap *regmap)
{
	u32 regval;
	int err;
...
static int nvsw_bmc_hid180_apply_hw_defaults(struct regmap *regmap)
{
	return regmap_write(regmap, NVSW_REG_AGGRCO_MASK_OFFSET, GENMASK(5, 0));
}

static int nvsw_bmc_hid191_apply_hw_defaults(struct regmap *regmap)
{
	return regmap_write(regmap, NVSW_REG_AGGRCO_MASK_OFFSET, GENMASK(6, 0));
}

static int nvsw_bmc_hid193_apply_hw_defaults(struct regmap *regmap)
{
	return regmap_write(regmap, NVSW_REG_AGGRCO_MASK_OFFSET, GENMASK(6, 0));
}
...
static int nvsw_bmc_hid162_platform_data_init(struct nvsw_core *nvsw_core)
{
	...
	for (i = 0; i < nvsw_core->mux_num; i++) {
		memcpy(&nvsw_core->mux_pdata[i], &nvsw_bmc_hid162_mux_data[i],
		       sizeof(nvsw_core->mux_pdata[i]));
		nvsw_core->mux_pdata[i].handle = nvsw_core;
		nvsw_core->mux_pdata[i].completion_notify = nvsw_bmc_hid162_completion_notify;
	}
	...
	nvsw_core->apply_hw_defaults = nvsw_bmc_hid162_apply_hw_defaults;
...
static int nvsw_bmc_hid176_platform_data_init(struct nvsw_core *nvsw_core)
{
	...
	for (i = 0; i < nvsw_core->mux_num; i++) {
		memcpy(&nvsw_core->mux_pdata[i], &nvsw_bmc_hid176_mux_data[i],
		       sizeof(nvsw_core->mux_pdata[i]));
		nvsw_core->mux_pdata[i].handle = nvsw_core;
		nvsw_core->mux_pdata[i].completion_notify = nvsw_bmc_hid162_completion_notify;
	}
...
static int nvsw_bmc_hid180_platform_data_init(struct nvsw_core *nvsw_core)
{
	...
	for (i = 0; i < nvsw_core->mux_num; i++) {
		memcpy(&nvsw_core->mux_pdata[i], &nvsw_bmc_hid180_mux_data[i],
		       sizeof(nvsw_core->mux_pdata[i]));
		nvsw_core->mux_pdata[i].handle = nvsw_core;
		nvsw_core->mux_pdata[i].completion_notify = nvsw_bmc_hid162_completion_notify;
	}
	...
	nvsw_core->mux_init = nvsw_bmc_hid162_mux_topology_init;
	nvsw_core->mux_exit = nvsw_bmc_hid162_mux_topology_exit;
	nvsw_core->apply_hw_defaults = nvsw_bmc_hid180_apply_hw_defaults;
...
static int nvsw_bmc_hid181_platform_data_init(struct nvsw_core *nvsw_core)
{
	...
	for (i = 0; i < nvsw_core->mux_num; i++) {
		memcpy(&nvsw_core->mux_pdata[i], &nvsw_bmc_hid181_mux_data[i],
		       sizeof(nvsw_core->mux_pdata[i]));
		nvsw_core->mux_pdata[i].handle = nvsw_core;
		nvsw_core->mux_pdata[i].completion_notify = nvsw_bmc_hid162_completion_notify;
	}
...
static int nvsw_bmc_hid189_platform_data_init(struct nvsw_core *nvsw_core)
{
	...
	for (i = 0; i < nvsw_core->mux_num; i++) {
		memcpy(&nvsw_core->mux_pdata[i], &nvsw_bmc_hid189_mux_data[i],
		       sizeof(nvsw_core->mux_pdata[i]));
		nvsw_core->mux_pdata[i].handle = nvsw_core;
		nvsw_core->mux_pdata[i].completion_notify = nvsw_bmc_hid162_completion_notify;
	}
...
static int nvsw_bmc_hid191_platform_data_init(struct nvsw_core *nvsw_core)
{
	...
	for (i = 0; i < nvsw_core->mux_num; i++) {
		memcpy(&nvsw_core->mux_pdata[i], &nvsw_bmc_hid191_mux_data[i],
		       sizeof(nvsw_core->mux_pdata[i]));
		nvsw_core->mux_pdata[i].handle = nvsw_core;
		nvsw_core->mux_pdata[i].completion_notify = nvsw_bmc_hid162_completion_notify;
	}
	...
	nvsw_core->mux_init = nvsw_bmc_hid162_mux_topology_init;
	nvsw_core->mux_exit = nvsw_bmc_hid162_mux_topology_exit;
	nvsw_core->apply_hw_defaults = nvsw_bmc_hid191_apply_hw_defaults;
...
static int nvsw_bmc_hid193_platform_data_init(struct nvsw_core *nvsw_core)
{
	...
	for (i = 0; i < nvsw_core->mux_num; i++) {
		memcpy(&nvsw_core->mux_pdata[i], &nvsw_bmc_hid193_mux_data[i],
		       sizeof(nvsw_core->mux_pdata[i]));
		nvsw_core->mux_pdata[i].handle = nvsw_core;
		nvsw_core->mux_pdata[i].completion_notify = nvsw_bmc_hid162_completion_notify;
	}
	...
	nvsw_core->mux_init = nvsw_bmc_hid162_mux_topology_init;
	nvsw_core->mux_exit = nvsw_bmc_hid162_mux_topology_exit;
	nvsw_core->apply_hw_defaults = nvsw_bmc_hid193_apply_hw_defaults;

	return 0;
}

static int nvsw_bmc_platform_data_init(struct nvsw_core *nvsw_core, enum nvsw_bmc_hid_type type)
{
	switch (type) {
	case HID162:
...
static int nvsw_bmc_hid162_probe(struct i2c_client *client)
{
	...
	enum nvsw_bmc_hid_type type;
	struct nvsw_bmc_device *bmc;
	...
	bmc = devm_kzalloc(&client->dev, sizeof(*bmc), GFP_KERNEL);
	if (!bmc)
		return -ENOMEM;

	nvsw_core = &bmc->core;
	nvsw_core->mux_pdata = bmc->mux_pdata;
...
MODULE_AUTHOR("Aaron Komisar <akomisar@nvidia.com>");
MODULE_DESCRIPTION("Nvidia BMC switch platform driver");
